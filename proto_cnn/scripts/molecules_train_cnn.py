...
import os
import scipy
from src.core.train_cnn_regression_sq import CNNTrainRegressionSQ
...
class CNNTrainRegressionSQ(CNNTrainRegression):
    ...
    def load_samples(self):
        ...
        test_set = np.random.uniform(-1,0,1000), np.random.uniform(-1,0,1000)
        test_set_x, test_set_y = self.prepare_dataset(test_set)
        valid_set = np.random.uniform(-1,0,500), np.random.uniform(-1,0,500)
        valid_set_x, valid_set_y = self.prepare_dataset(valid_set)
        train_set = np.random.uniform(-1,0,500), np.random.uniform(-1,0,500)
